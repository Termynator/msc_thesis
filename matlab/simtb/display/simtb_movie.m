...
pp=get(h,'Position');
set(h,'Position',[pp(1),pp(2),voxelsize*sP.nV,voxelsize*sP.nV]); % size the figure window
set(gca,'Position',[0 0 1 1]); % make image full size
amax = max(A(:));
for ii = 1:size(A,1)
    imagesc(squeeze(A(ii,:,:)), [0 amax]); axis xy; axis square; axis off;pp=get(h,'Position');
    %title(num2str(ii));
    tt = text(0.03*sP.nV, 0.98*sP.nV, ['TR ' num2str(ii)]); set(tt,'Color',[1 1 1]);
    tt = text(0.85*sP.nV, 0.98*sP.nV, [num2str(ii*sP.TR) 's']); set(tt,'Color',[1 1 1]);
    tt = text(0.03*sP.nV, 0.03*sP.nV, [sP.prefix]); set(tt,'Color',[1 1 1]);
    if ~isempty(sub)
        tt = text(0.85*sP.nV, 0.03*sP.nV, ['sub ' num2str(sub)]); set(tt,'Color',[1 1 1]);
    end
    if avi 
        F = getframe(h);
